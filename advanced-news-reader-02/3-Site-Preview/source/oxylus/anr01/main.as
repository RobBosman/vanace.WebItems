...
class oxylus.anr01.main extends MovieClip
{
	...
	public function cont()
	{
		...
		/*
		 * this is only for the preview version, this will activate the main bg, 
		 * it will resize it to the entire stage and it will center the app on center
		 * 
		 * disable this if you want to use the component into another project
		*/
	...
	private function previewMode() {
		...
		_root["bg"]._x = _root["bg"]._y = 0;
		this._x = Math.round(Stage.width / 2 - settings.totalWidth / 2);
		this._y = Math.round(Stage.height / 2 - (settings.totalHeight + settings.maskHeight) / 2);
	}
